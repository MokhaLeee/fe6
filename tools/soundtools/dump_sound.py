...
def dump_sound_header(song_header):
...
def dump_one_song(rom_data, addr, name, wav_groups):
	...
	if song_header.trackCount != 0:
		...
		for i in range(song_header.trackCount):
			...
			# print(".align 2")
			print(f"{song_header.tracks[i].name}: @ {(track.addr | 0x08000000):08X}")
			dump_sound_track(rom_data, song_header, i, max_len)
			print("")

	print(".align 2")
	print(f".global {name} @ {(addr | 0x08000000):08X}")
	print(f"{name}:")

	print("@ ****************************** header ******************************")
	song_header = dump_sound_header(song_header)
	print("")
