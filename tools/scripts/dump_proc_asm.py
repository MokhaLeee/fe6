...
def main(args):
	...
	try:
		end_offset = 0x1FFFFFF & int(args[1], base = 0)
	except IndexError:
		end_offset = offset

	elf_name = "fe6.elf"
	rom_name = "fe6-base.gba"

	with open(elf_name, 'rb') as f:
		syms = { addr: name for addr, name in symbols.from_elf(f) }

	while True:
		offset = dump_one_part(offset, rom_name, syms)
		print("")
